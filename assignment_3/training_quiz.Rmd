...
# Your code here
data_list <- map(files_list, \(x) read_csv(x))
data_list
...
map(data_list_named, \(x) head(x, 5))
...
mdn_df <- wide_df |>
  ungroup() |> 
  rowwise() |>  # c_across is designed to work with rowwise(), removed group since we're doing rowwise operations
...
functions_list <- list(log2, 
                       ...
                       \(x) (x - median(x, na.rm = T))/range(x, na.rm = T))
...
processed_df <- processed_df |>
  bind_cols(map2_dfc(functions_list, c('log2', '(x-mean)/sd', '(x-median)/range'), function(func, abbr) {
                map_dfc(processed_df |> select(A:C), \(x) func(x)) |> 
                  setNames(paste0(colnames(processed_df |> select(A:C)), "_", abbr))
}))

processed_df <- processed_df |>
  rename_with(\(x) paste0(x, '_raw'), A:C)
...
processed_df |> 
  ...
  separate_wider_delim(Name, "_", names = c('Category', 'Type'), cols_remove = F) |> 
